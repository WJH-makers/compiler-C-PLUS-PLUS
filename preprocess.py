...
"""
这是一个简单的 C/C++ 风格预处理器实现。

功能:
1. 读取指定的源代码文件。
2. 处理行连接符 '\'。
3. 移除 /* ... */ 和 // ... 风格的注释。
4. 处理部分预处理指令: #define, #undef, #if, #ifdef, #ifndef, #elif, #else, #endif。
   根据条件编译指令（#if 等）决定代码块的活跃状态，只处理活跃块中的 #define/#undef 和普通代码。
5. 执行对象式宏和函数式宏的展开（支持有限次迭代，防止死循环）。
6. 忽略 #include 指令（不实际包含文件内容）。
7. 跳过非活跃条件编译块中的代码行。
8. 对活跃块中遇到的 #error, #warning, #pragma 等指令简单记录警告并忽略。
9. 在活跃块中遇到未知指令则报错。

输出:
1. 打印预处理后的代码到标准输出。
2. 打印一个行号映射表，显示预处理后代码的每一行对应原始文件的哪一行。

使用方法:
在命令行中运行此脚本，并提供一个输入文件路径作为参数：
python 你的脚本文件名.py <输入文件路径>

示例:
python basic_preprocessor.py main.cpp

*** 这是一个基础实现，可能不支持所有复杂的预处理器特性、嵌套宏的复杂交互或边缘情况。
"""
